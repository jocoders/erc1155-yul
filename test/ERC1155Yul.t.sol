...
contract Receiver is ERC1155TokenReceiver {
  ...
  // function onERC1155BatchReceived1(address _from, address _to, uint256[] calldata _ids) public returns (bytes4) {
  //   console.log('***************!!!!!!!!!!!_onERC1155BatchReceived_!!!!!!!!!!!!***************');
  //   from = _from;
  //   to = _to;
  //   ids = _ids;
...
contract ERC1155YulTest is Test {
  ...
  //   (uint256[] memory _ids, uint256[] memory _amounts) = _batchMint(cData);
  ...
  /* -------------------------- Test helpers -------------------------- */
  function _batchMint(bytes memory cData) public returns (uint256[] memory _ids, uint256[] memory _amounts) {
    console.log('***************!!!!!!!!!!!_DATA_!!!!!!!!!!!!***************');
    console.logBytes(cData);
    console.log('***************!!!!!!!!!!!_DATA_!!!!!!!!!!!!***************');
    ...
    //_checkBatchStoredValues(address(receiver), _ids, _amounts, false);
  }

  function _checkBatchStoredValues(address to, uint256[] memory _ids, uint256[] memory _amounts, bool _isZero) private {
